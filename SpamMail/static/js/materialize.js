...
(function (window) {
    var lastTime = 0, vendors = ["webkit", "moz"], requestAnimationFrame = window.requestAnimationFrame,
        cancelAnimationFrame = window.cancelAnimationFrame, i = vendors.length;
        ...
        _createClass(Modal, [{
            ...
            key: "animateOut", value: function animateOut() {
                ...
                if (this.$el[0].classList.contains("bottom-sheet")) {
                    ...
                    Vel(this.$el[0], {top: this.options.startingTop, opacity: 0, scaleX: .7}, exitVelocityOptions)
                }
            }
        }, {
            ...
            key: "init", value: function init($els, options) {
                var arr = [];
                $els.each(function () {
    ...
    var methods = {
        init: function (options) {
            ...
            return this.each(function (i) {
                ...
                var $active, $content, $links = $this.find("li.tab a"), $tabs_width = $this.width(),
                    $tabs_content = $(), $tabs_wrapper,
                    $tab_width = Math.max($tabs_width, $this[0].scrollWidth) / $links.length, $indicator,
                    index = prev_index = 0, clicked = false, clickedTimeout, transition = 300;
                ...
                if (options.swipeable) {
                    ...
                    $(".tabs-content.carousel").carousel({
                        fullWidth: true, noWrap: true, onCycleTo: function (item) {
                            if (!clicked) {
        ...
        return this.each(function () {
            ...
            origin.on({
                "mouseenter.tooltip": function (e) {
                    ...
                    setTimeout(function () {
                        if (started !== true) {
                            tooltipEl.velocity({opacity: 0, translateY: 0, translateX: 0}, {
                                duration: 225,
                                queue: false
                            });
...
(function ($) {
    ...
    var Toast = function () {
        ...
        _createClass(Toast, [{
            key: "createToast", value: function createToast() {
            ...
            key: "_animateIn", value: function _animateIn() {
                Vel(this.el, {top: 0, opacity: 1}, {duration: 300, easing: "easeOutCubic", queue: false})
            }
        }, {
            key: "setTimer", value: function setTimer() {
                var _this3 = this;
            ...
            key: "_createContainer", value: function _createContainer() {
            ...
            key: "_removeContainer", value: function _removeContainer() {
            ...
            key: "_onDragStart", value: function _onDragStart(e) {
            ...
            key: "_onDragEnd", value: function _onDragEnd(e) {
                if (!!Toast._draggedToast) {
                    var toast = Toast._draggedToast;
                    toast.panning = false;
...
(function ($) {
    ...
    function findElements(top, right, bottom, left) {
        ...
        $.each(elements, function (i, element) {
            if (element.height() > 0) {
                var elTop = element.offset().top, elLeft = element.offset().left, elRight = elLeft + element.width(),
    ...
    function onScroll(scrollOffset) {
        ++ticks;
        var top = jWindow.scrollTop(), left = jWindow.scrollLeft(), right = left + jWindow.width(),
    ...
    $.scrollSpy = function (selector, options) {
        var defaults = {
            throttle: 100, scrollOffset: 200, activeClass: "active", getActiveElement: function (id) {
                return 'a[href="#' + id + '"]'
            }
        };
        options = $.extend(defaults, options);
...
(function ($) {
    $(document).ready(function () {
        ...
        $.fn.autocomplete = function (options) {
            ...
            return this.each(function () {
                var $input = $(this);
                var data = options.data, count = 0, activeIndex = -1, oldVal,
                ...
                if (!$.isEmptyObject(data)) {
                    ...
                    $input.off("keydown.autocomplete").on("keydown.autocomplete", function (e) {
                        var keyCode = e.which, liElement, numItems = $autocomplete.children("li").length,
    ...
    $.fn.material_select = function (callback) {
        $(this).each(function () {
            ...
            var options = $('<ul id="select-options-' + uniqueID + '" class="dropdown-content select-dropdown ' + (multiple ? "multiple-select-dropdown" : "") + '"></ul>'),
                selectChildren = $select.children("option, optgroup"), valuesSelected = [], optionsHover = false;
...
(function (factory) {
    ...
    function PickerConstructor(ELEMENT, NAME, COMPONENT, OPTIONS) {
        ...
        var IS_DEFAULT_THEME = false, STATE = {id: ELEMENT.id || "P" + Math.abs(~~(Math.random() * new Date))},
...
(function (factory) {
    ...
    function DatePicker(picker, settings) {
        var calendar = this, element = picker.$node[0], elementValue = element.value,
            elementDataValue = picker.$node.data("value"), valueString = elementDataValue || elementValue,
            formatString = elementDataValue ? settings.formatSubmit : settings.format, isRTL = function () {
    ...
    DatePicker.prototype.navigate = function (type, value, options) {
        var targetDateObject, targetYear, targetMonth, targetDate, isTargetArray = $.isArray(value),
            isTargetObject = $.isPlainObject(value), viewsetObject = this.item.view;
    ...
    DatePicker.prototype.validate = function (type, dateObject, options) {
        var calendar = this, originalDateObject = dateObject,
            interval = options && options.interval ? options.interval : 1, isFlippedBase = calendar.item.enable === -1,
            hasEnabledBeforeTarget, hasEnabledAfterTarget, minLimitObject = calendar.item.min,
            maxLimitObject = calendar.item.max, reachedMin, reachedMax,
    ...
    DatePicker.prototype.nodes = function (isOpen) {
        var calendar = this, settings = calendar.settings, calendarItem = calendar.item, nowObject = calendarItem.now,
            selectedObject = calendarItem.select, highlightedObject = calendarItem.highlight,
            viewsetObject = calendarItem.view, disabledCollection = calendarItem.disable,
            minLimitObject = calendarItem.min, maxLimitObject = calendarItem.max,
            ...
            createMonthNav = function (next) {
                ...
                if (numberYears) {
                    var minYear = minLimitObject.year, maxYear = maxLimitObject.year,
                        lowestYear = focusedYear - numberYears, highestYear = focusedYear + numberYears;
...
(function () {
    ...
    var touchSupported = "ontouchstart" in window, mousedownEvent = "mousedown" + (touchSupported ? " touchstart" : ""),
    ...
    var dialRadius = 135, outerRadius = 105, innerRadius = 80, tickRadius = 20, diameter = dialRadius * 2,
    ...
    function ClockPicker(element, options) {
        var popover = $(tpl), plate = popover.find(".clockpicker-plate"), holder = popover.find(".picker__holder"),
            hoursView = popover.find(".clockpicker-hours"), minutesView = popover.find(".clockpicker-minutes"),
            amPmBlock = popover.find(".clockpicker-am-pm-block"), isInput = element.prop("tagName") === "INPUT",
            input = isInput ? element : element.find("input"), label = $("label[for=" + input.attr("id") + "]"),
            self = this;
        this.id = uniqueId("cp");
        ...
        function mousedown(e, space) {
            var offset = plate.offset(), isTouch = /^touch/.test(e.type), x0 = offset.left + dialRadius,
                y0 = offset.top + dialRadius, dx = (isTouch ? e.originalEvent.touches[0] : e).pageX - x0,
                dy = (isTouch ? e.originalEvent.touches[0] : e).pageY - y0, z = Math.sqrt(dx * dx + dy * dy),
                moved = false;
            ...
            $doc.off(mousemoveEvent).on(mousemoveEvent, function (e) {
                e.preventDefault();
                var isTouch = /^touch/.test(e.type), x = (isTouch ? e.originalEvent.touches[0] : e).pageX - x0,
            ...
            $doc.off(mouseupEvent).on(mouseupEvent, function (e) {
                ...
                e.preventDefault();
                var isTouch = /^touch/.test(e.type), x = (isTouch ? e.originalEvent.changedTouches[0] : e).pageX - x0,
    ...
    ClockPicker.prototype.locate = function () {
        var element = this.element, popover = this.popover, offset = element.offset(), width = element.outerWidth(),
            height = element.outerHeight(), align = this.options.align, self = this;
        popover.show()
    };
    ...
    ClockPicker.prototype.resetClock = function (delay) {
        var view = this.currentView, value = this[view], isHours = view === "hours",
            ...
            radius = isHours && value > 0 && value < 13 ? innerRadius : outerRadius, x = Math.sin(radian) * radius,
            y = -Math.cos(radian) * radius, self = this;
    ...
    ClockPicker.prototype.setHand = function (x, y, roundBy5, dragging) {
        var radian = Math.atan2(x, -y), isHours = this.currentView === "hours",
            unit = Math.PI / (isHours || roundBy5 ? 6 : 30), z = Math.sqrt(x * x + y * y), options = this.options,
            inner = isHours && z < (outerRadius + innerRadius) / 2, radius = inner ? innerRadius : outerRadius, value;
        if (options.twelvehour) {
            radius = outerRadius
        }
        if (radian < 0) {
        ...
        var cx1 = Math.sin(radian) * (radius - tickRadius), cy1 = -Math.cos(radian) * (radius - tickRadius),
...
(function ($) {
    ...
    function updateCounter() {
        var maxLength = +$(this).attr("data-length"), actualLength = +$(this).val().length,
